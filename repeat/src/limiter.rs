...
impl Limiter {
  ...
  pub fn process(&mut self, input: f32, is_on: bool) -> f32 {
    if is_on {
      ...
      (delay_output * limiter_gain).clamp(-self.limit, self.limit)
    } else {
      input
    }
  }
  ...
  fn read_from_buffer(&self) -> f32 {
    self.buffer[self.buffer_index]
  }

  fn get_gain_reduction(&self, input: f32) -> f32 {
    let gain = input.abs();
    if gain > self.limit {
